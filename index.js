...
// ==========================================
// 8. 👁️ Omni-Vision 全視之眼沙盒 UI (Absolute Vision)
// ==========================================
let omniRenderTimeout = null;
let omniMappings = [];
let isSyncLocked = true;
let activeScrollDriver = null; // 'left' 或 'right'
...
async function showOmniVisionUI() {
    ...
    // 綁定滾動事件 (駕駛員模式：徹底解決迴圈死鎖)
    const leftPane = document.getElementById('omni-left-pane');
    const rightPane = document.getElementById('omni-right-pane');
    
    leftPane.addEventListener('mouseenter', () => { activeScrollDriver = 'left'; });
    rightPane.addEventListener('mouseenter', () => { activeScrollDriver = 'right'; });
    
    leftPane.addEventListener('scroll', () => {
        requestCanvasUpdate();
        if (!isSyncLocked || activeScrollDriver !== 'left') return;
        const pct = leftPane.scrollTop / (leftPane.scrollHeight - leftPane.clientHeight || 1);
        rightPane.scrollTop = pct * (rightPane.scrollHeight - rightPane.clientHeight);
    }, { passive: true });
    ...
    rightPane.addEventListener('scroll', () => {
        requestCanvasUpdate();
        if (!isSyncLocked || activeScrollDriver !== 'right') return;
...
async function renderOmniVision(state) {
    ...
    // 4. 觸發畫布重繪 (ResizeObserver 會自動接管後續的精準對齊)
    requestAnimationFrame(() => {
        resizeCanvas();
        requestCanvasUpdate();
    });
...
function updateOmniCanvas() {
    ...
    omniMappings.forEach(m => {
        ...
        // 實時獲取 DOM 元素的絕對位置 (徹底解決脫位與聚集問題)
        const lNode = m.left !== -1 ? document.getElementById(`omni-left-node-${m.left}`) : null;
        const rNode = m.right !== -1 ? document.getElementById(`omni-right-node-${m.right}`) : null;
        ...
        // 可視範圍檢測 (允許在畫布外一點點繪製，防止線條突然消失)
        if (m.type === 'deleted' && lNode) {
            if (startY > -100 && startY < height + 100) isVisible = true;
        } else if (m.type.startsWith('new_') && rNode) {
    ...
    try {
        ...
        <div class="inline-drawer" id="ds-v55-opt-drawer">
            <div class="inline-drawer-toggle inline-drawer-header" style="background: linear-gradient(90deg, rgba(0,229,255,0.1) 0%, rgba(0,0,0,0) 100%); border-left: 3px solid var(--ds-cyan);">
                <b style="color:var(--ds-cyan); text-shadow: 0 0 8px rgba(0,229,255,0.3);"><span class="fa-solid fa-microchip"></span> DeepSeek 绝对真理优化器 (v55)</b>
...
jQuery(async () => {
    try {
        ...
        Logger.log('══════ 🚀 DeepSeek 绝对真理优化器 v55 引擎上线 ══════', LogLevels.BASIC);
    } catch (e) {
