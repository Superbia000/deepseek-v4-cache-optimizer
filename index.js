...
const injectCSS = () => {
    ...
    style.innerHTML = `
        ...
        /* 完美修復 1：移除 ds-virtual-list，確保絕對座標精準映射 */
...
let omniIsScrolling = false;
let omniScrollTimeout = null;
...
function handleOmniScroll() {
    omniIsScrolling = true;
    requestCanvasUpdate();
    if (omniScrollTimeout) clearTimeout(omniScrollTimeout);
    omniScrollTimeout = setTimeout(() => {
        omniIsScrolling = false;
        requestCanvasUpdate();
    }, 150);
}
...
async function showOmniVisionUI() {
    ...
    let ignoreLeftScroll = false;
    let ignoreRightScroll = false;
    ...
    leftPane.addEventListener('scroll', function() {
        handleOmniScroll();
        if (!isSyncLocked || ignoreLeftScroll) return;
        ignoreRightScroll = true; 
        syncScroll('left');
        setTimeout(() => { ignoreRightScroll = false; }, 20);
    }, {passive: true});

    rightPane.addEventListener('scroll', function() {
        handleOmniScroll();
        if (!isSyncLocked || ignoreRightScroll) return;
        ignoreLeftScroll = true; 
        syncScroll('right');
        setTimeout(() => { ignoreLeftScroll = false; }, 20);
    }, {passive: true});
...
function renderOmniLeftPane(leftArray) {
    ...
    leftArray.forEach((node, idx) => {
        ...
        el.className = `ds-node-card ds-node-hit`; 
...
window.jumpToOmniNode = function(side, id) {
    ...
    if (targetPane && targetNode) {
        ...
        syncLockTimeout = setTimeout(() => {
            isSyncLocked = $('#omni-btn-sync').hasClass('active');
        }, 800);
    }
};
...
async function renderOmniVision(state) {
    ...
    rightArray.forEach((node, idx) => {
        ...
        if (isNew) {
            ...
            else if (node._omniCat === 'patch_time') { cardClass = 'ds-node-time'; newLabel = '<span style="color:var(--ds-orange); font-weight:bold; margin-left:6px;">(TIME SYNC)</span>'; }
...
function updateOmniCanvas() {
    ...
    // 完美修復 2：動態 GPU 降載引擎 (Dynamic GPU Throttling)
    // 當正在滾動，或者連線數量過多時，關閉極度消耗效能的 shadowBlur 與漸變色
    const fastMode = omniIsScrolling || omniMappings.length > 50;
    if (fastMode) {
        ctx.shadowBlur = 0; 
    } else {
        ctx.shadowBlur = 5;
